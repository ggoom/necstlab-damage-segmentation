...
def add_data_to_storage(data_storage, truth_storage, subject_data, n_channels, truth_dtype):
    sys.stdout.write(str(np.asarray(subject_data[:n_channels])[np.newaxis][np.newaxis].shape))
    data_storage.append(np.asarray(subject_data[:n_channels])[np.newaxis][np.newaxis])
