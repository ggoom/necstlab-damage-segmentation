...
def prepare_dataset(gcp_bucket, config_file):
    ...
    sys.stdout.write(str([str(p) for p in processed_data_remote_source.iterdir()]))
    label_type = 'masks' if 'masks' in [str(p) for p in processed_data_remote_source.iterdir()] else 'annotations'
